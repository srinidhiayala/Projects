...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    while win_left_edge < INIT_SEQNO + content_len:
    	readable, writable, errors = select.select([cs], [], [], RTO)
    	if cs in readable:
    		message, address = cs.recvfrom(100)
    		fullMessage = Msg.deserialize(message)
    		ackMsg = fullMessage.ack
    		if ackMsg > win_left_edge:
