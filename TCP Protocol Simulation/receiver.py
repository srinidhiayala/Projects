...
def part5(username):
    ...
    if username[0:3] == "UN@":
    	msgList = username
    	if "\n" in username:
    		msgList = username.split("\n")
    		if " " in msgList[0]:
    			msgList = msgList[0].split(" ")
    	elif " " in username:
    		msgList = username.split("\n")
    		if "\n" in msgList[0]:
    			msgList = msgList[0].split("\n")
    	msgList = msgList[0].split('@') 
    	answer = part5CheckFile(msgList)
    	return answer
    else:
    	return "Username not found"

def part5CheckFile(msgList):
	...
	with open("UN.txt", "a+") as f: 
		...
		for line in f:
			...
			if msgList[1] == line:
				flag = True
				answer = "Username found: " +  msgList[1]
				return answer
		if flag == False:
			f.write(msgList[1] + "\n")
			return "Username not found"
...
def receiver(ss, ooo_enabled):
    ...
    while last_acked <= last_seq_expected:
        ...
        if((msg.seq <= 13 and msg.seq > 5) and flag == False):
        	flag = True
        	answer = part5(msg.msg)
